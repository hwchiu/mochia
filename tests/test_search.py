...
from app.database import Video, Summary, Transcript, Classification
...
class TestRebuildFtsIndex:
    ...
    def test_rebuild_key_points_new_format(self, db_session):
        ...
        db_session.add(Video(
            id=vid_id, filename="kp_test.mp4", original_filename="kp_test.mp4",
            file_size=100, status="completed",
        ))
        db_session.add(Summary(
            id=uuid.uuid4().hex, video_id=vid_id, summary="摘要",
            key_points=kp,
        ))
        db_session.commit()
        # 不應拋出例外
    ...
    def test_rebuild_key_points_old_string_format(self, db_session):
        ...
        db_session.add(Video(
            id=vid_id, filename="kp_str.mp4", original_filename="kp_str.mp4",
            file_size=100, status="completed",
        ))
        db_session.add(Summary(
            id=uuid.uuid4().hex, video_id=vid_id, summary="摘要",
    ...
    def test_rebuild_transcript_truncated_to_8000(self, db_session):
        ...
        db_session.add(Video(
            id=vid_id, filename="long.mp4", original_filename="long.mp4",
            file_size=100, status="completed",
        ))
        db_session.add(Transcript(
            id=uuid.uuid4().hex, video_id=vid_id, content=long_transcript,
        ))
        db_session.add(Summary(
            id=uuid.uuid4().hex, video_id=vid_id, summary="s", key_points="[]",
        ))
        db_session.commit()
