    ...
    resultsEl.innerHTML = data.items.map(item => `
      <div class="card" style="margin-bottom:12px">
        <div style="display:flex;align-items:flex-start;gap:12px">
          <div style="flex:1;min-width:0">
            <a href="/video/${item.id}${item.start_sec != null ? `?t=${encodeURIComponent(item.start_sec)}` : ""}" style="font-weight:700;font-size:14px;color:var(--primary,#4f46e5);text-decoration:none">
          ...
          <a href="/video/${item.id}${item.start_sec != null ? `?t=${encodeURIComponent(item.start_sec)}` : ""}" class="btn btn-ghost btn-sm" style="white-space:nowrap">${item.timestamp ? "跳到片段 →" : "查看 →"}</a>
        </div>
      </div>
    `).join("");
  } catch (e) {
